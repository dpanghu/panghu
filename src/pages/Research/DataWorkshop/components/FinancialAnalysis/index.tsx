...
const FinancialAnalysis: React.FC<TProps> = ({}) => {
    ...
    <div style={{ padding: '20px 24px' }}>
      <div className={styles.container}>
        <div className={styles.left}>
          <div className={styles.title}>金融分析</div>
        ...
        <div className={styles.right}>
          ...
          <div className={styles.detail}>
            <div>外汇储备 (万美元)</div>
            <div>{(currentList?.titelMap && currentList?.titelMap[2]?.output) || 0}</div>
            <div>
              上年: {(currentList?.titelMap && currentList?.titelMap[2]?.butOneOutput) || 0}（
              {(currentList?.titelMap && currentList?.titelMap[2]?.growthRate) || 0}）
            </div>
          </div>
        </div>
      </div>
