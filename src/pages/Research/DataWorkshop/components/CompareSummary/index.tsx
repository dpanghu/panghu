  ...
  const getToken = async () => {
    ...
    fetch(`https://data.yonyoucloud.com/fsb/seentao/token`, {
      ...
      result.then(async (res: any) => {
        ...
        await fetch(
          `https://data.yonyoucloud.com/fsb/seentao/direct?token=${aaa.data}&key=CompareSummary`,
          {
            method: 'GET',
  ...
  return (
    <div style={{ width: '100%', height: '100%' }}>
      {Number.isNaN(Number(userName)) ? (
        ...
        <iframe frameBorder="no" src={url} title="对标情况页面" width={'100%'} height={'100%'} />
      )}
    </div>
  );
};
