...
const AiLPR: React.FC = ({ }) => {
    ...
    const copyText = () => {
        const wordsText = state.IdentifyData.map(item => `${item.number}\n${colorMap[item.color]}`).join('\n');
    ...
    const postTextRecognition = async (url: any, picUid: any, rotationAngle: any, isPreset: any) => {
        state.isLoading = true;
        try {
            const recognitionResults = await CarRecognition({ url, picUid, rotationAngle, isPreset });
                    ...
                    {state.isrec && state.preData.length > 0 && (<div className={styles.right}>
                        <div className={state.isLoading || state.IdentifyData.length === 0 ? styles.lRightResult : styles.rightResult}>
                            ...
                            <div className={styles.rightItems}>
                                ...
                                {state.IdentifyData.length > 0 && <div className={styles.rightItem}>
                                    <div className={styles.rightNumber}>内容</div>
                                    {state.IdentifyData?.map((item: any, index: any) => (<div className={styles.rightItem}>
                                        <div className={styles.rightNumber}>{item.number}</div>
                                    </div>))}
                                </div>}
                                {state.IdentifyData.length > 0 && <div className={styles.rightItem}>
                                    <div>颜色</div>
                                    {state.IdentifyData?.map((item: any, index: any) => (<div className={styles.rightItem}>
                                        <div>{colorMap[item.color]}</div>
                                    </div>))}
                                </div>}
                            </div>
