  ...
  return (
    <div className={styles.container} id="caseViewContainerMain">
      <div className={styles.header}>
        <Header oneTitle="案例设置" seTitle="查看案例介绍" oneRoutePath="/case" />
        <div className={styles.action}>
          <Button
            onClick={() =>
              caseDetail.casesState ? message.warning('案例已发布，不能删除') : setDeleteModal(true)
            }
          >
            删除
          </Button>
          {caseDetail.casesState === 1 ? (
            <Button onClick={() => setOpen(true)}>编辑</Button>
          ) : (
            <Button onClick={handleEdit}>编辑</Button>
          )}
          <Button type="primary" onClick={() => setConfirmModal(true)}>
            {caseDetail.casesState ? '取消发布' : '发布'}
          </Button>
        </div>
      </div>
        ...
        <div className={styles.banner}>
          ...
          <div className={styles.detail}>
            <span className={styles.titleBox}>
              <div className={styles.title}> {caseDetail.casesName}</div>
              <div style={{ height: '20px', width: '40px', display: 'inline-block' }}>
