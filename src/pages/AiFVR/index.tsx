...
const AiFVR: React.FC = ({}) => {
  ...
  const copyText = () => {
    const wordsText = state.IdentifyData.map(
      (item: any, index: any) =>
        `${index + 1}\t${item.name}\t${item.score.toFixed(3)}`,
    ).join('\n');
    navigator.clipboard
      .writeText(wordsText)
      .then(() => {
  ...
  const postTextRecognition = async (url: any, picUid: any, isPreset: any) => {
    ...
    try {
      const recognitionResults = await FruitRecognition({
        url,
        picUid,
        isPreset,
      });
    ...
    Modal.confirm({
      ...
      onOk() {
        deletePic({ picUid: state.isSelect }).then((res) => {
          ...
          selectImage(item.picUid, item.url, item.note, item.isPreset);
          getImageLists();
