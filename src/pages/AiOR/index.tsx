...
const AiOR: React.FC = ({ }) => {
    ...
    const copyText = () => {
        const wordsText = state.IdentifyData.map((item) => `${item.keyword}\t${item.score}`).join('\n');
        navigator.clipboard
            .writeText(wordsText)
            .then(() => {
    ...
    const postTextRecognition = async (
        ...
        state.isLoading = true;
        try {
            const recognitionResults = await ObjectRecognition({
                url,
                picUid,
                rotationAngle,
                isPreset,
                        ...
                        <div className={styles.right}>
                            ...
                            >
                                ...
                                <div className={styles.rightItems}>
                                    ...
                                    {state.IdentifyData.length > 0 && <div className={styles.rightItem}>
                                        <div className={styles.rightNumber}>物体</div>
                                        {state.IdentifyData?.map((item: any, index: any) => (<div className={styles.rightItem}>
